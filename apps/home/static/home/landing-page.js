...
$(document).ready(function(){
    ...
    $('.slider-track').mouseover(function(){
        $('.tooltip').css({'opacity':'1'});
    });
    $('.slider-track').mouseleave(function(){
        $('.tooltip').css({'opacity':'0'});
    });
    $('.span2').slider('setValue',[$('#id_min_rent').val(),$('#id_max_rent').val()])
    $('.span2').slider().on('slideStop',function(){
    ...
    $('.select2-search__field').toggleClass('select-2-search__field');
    ...
    $('body').click(function(e){
        ...
        if(!filters.is(e.target) && !filter_button.is(e.target) && filters.has(e.target).length==0 && filter_button.has(e.target).length==0 && !$(e.target).hasClass('select2-selection__choice__remove')){
            $('.filters').addClass('hide');
            $('.filter-arrow-up').addClass('hide');
        }
    })
});